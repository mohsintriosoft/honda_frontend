...
export default function Callbacks() {
  ...
  const fetchCallbacks = useCallback(
    async (silent = false) => {
      ...
      try {
        ...
        console.error(err);
        if (!silent) setError(apiErrorMessage(err, "Couldn't load callbacks. Try refreshing."));
      } finally {
  ...
  const updateStatus = async (row: CallbackRow, status: "completed" | "cancelled") => {
    ...
    setError(null);
    try {
      const res = await server_patch_data(patch_callback(row.id), {
        status,
        handledAt: new Date().toISOString(),
      });
      if (res?.success === false) throw { response: { data: res } };

      const updated: CallbackRow | undefined = res?.callback;
      const stillMatches = statusFilter === "all" || statusFilter === status;

      setRows((prev) =>
        stillMatches
          ? prev.map((r) =>
              r.id === row.id
                ? (updated ?? { ...r, status, handledAt: new Date().toISOString() })
                : r,
            )
          : prev.filter((r) => r.id !== row.id),
      );

      // Backend didn't return the row -> reload quietly so handledBy /
      // handledAt show the server's values.
      if (!updated && stillMatches) void fetchCallbacks(true);
    } catch (err) {
      console.error(err);
      setError(apiErrorMessage(err, "Couldn't update that callback. Try again."));
    } finally {
      setUpdatingId(null);
    }
  };
