...
export default function IntentDetailsPage() {
  ...
  useEffect(() => {
    ...
    server_get_data(get_intent_summary(id))
      .then((res) => {
        if (cancelled) return;
  ...
  // Turn-level table — paginated server-side, refetched on id/view/search/page.
  useEffect(() => {
    ...
    server_get_data(get_intent_turns(id), {
      view,
      search: q || undefined,
      page,
      page_size: PAGE_SIZE,
  ...
  const headerLabel = summary?.label ?? (id ? `Intent ${id}` : "Intent");
